...
# def get_irises_location(frame_gray, glasses=False):
#     if glasses:
#         eye_cascade = cv2.CascadeClassifier(GLASS_EYE_LIB)
#     else:
...
#         cv2.circle(frame, (ex, ey), 1, (255, 255, 255), 2)
#         cv2.circle(frame, (ew, eh), 1, (0,0,0), 2)
#         iris_w = int(ex + float(ew / 2))
#         iris_h = int(ey + float(eh / 2))
#         irises.append([np.float32(iris_w), np.float32(iris_h)])

#     print "Number of eyes: " + str(len(irises))
...
def eyes(gray, old_gray, irises, blinks, blink_in_previous, landmark):
    ...
    if right_eye_frame.shape[0] > 0 and right_eye_frame.shape[1] > 0:
        ...
        # _, binary_right_eye_frame = cv2.threshold(gray_right_eye_frame, 55, 255, cv2.THRESH_BINARY)

        # (uh, uhc) = (100, 200)
        # cv2.imwrite("canny/right_canny_sample.jpg", binary_right_eye_frame)
        # for uh in range(20, 300, 20):
        #     for uhc in range(50, 450, 20):
        #         gray_right_eye_frame =  cv2.Canny(binary_right_eye_frame, uh, uhc)
        #         cv2.imwrite("canny/right_" + "canny_" + str(uh) + "_" + str(uhc) + ".jpg", gray_right_eye_frame)
        # print "OK byebye!!!!"
        # while True:
        #     if uh == -1:
        #         uh = 0

        # binary_right_eye_frame = cv2.Canny(binary_right_eye_frame, uh, uhc)

        _, contours, hierarchy = cv2.findContours(binary_right_eye_frame,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
        # print contours
        i = 0
        for cnt in contours:
        ...
        #if there are 3 or more blobs, pick the largest blob
        #if there are 2 blob, take the second largest
        #if there are 1 or less blobs, do nothing
        # print (len(contours))
        bestBlob = -1
        if len(contours) >= 2:
            # cv2.imwrite("right_eye_" + str(len(contours)) + ".jpg", binary_right_eye_frame)
            # for i in range(len(contours)):
            #     cv2.drawContours(right_eye_frame, contours, i, ((i % 3 ==0)*255,(i % 3 == 1)*255, (i % 3 == 2)*255))
            # cv2.imshow("right_contours", cv2.resize(right_eye_frame, (0, 0), fx=10, fy=10))
            # cv2.waitKey(0)
            maxArea = -1
            maxIndex = 0
            i = 0
        ...
        #     binary_right_eye_frame, cv2.HOUGH_GRADIENT, 3, 100, 100, 200)
        ...
        #         cv2.circle(right_eye_frame, (x, y), r, (0, 255, 0), 4)


        # counter = 0
        # for y in range(gray_right_eye_frame.shape[0]):
        #     for x in range(gray_right_eye_frame.shape[1]):
        #         if gray_right_eye_frame[y][x] != 0:
        #             rpx += x; rpy += y
        #             counter += 1
        # if counter > 0:
        #     rpx /= counter; rpy /= counter
        cv2.imshow("right eye raw", cv2.resize(binary_right_eye_frame, (0, 0), fx=10, fy=10))
        for i in range(len(contours)):
            cv2.drawContours(right_eye_frame, contours, i, ((i % 3 ==0)*255,(i % 3 == 1)*255, (i % 3 == 2)*255))
        # # cv2.circle(right_eye_frame, (int(rpx),int(rpy)), 1, (255,0,255), 1)
        cv2.imshow("right eye after", cv2.resize(right_eye_frame, (0, 0), fx=10, fy=10))
